 ...
 * while uploading and a check mark once done. An upload can't be cancelled once started, so
 * the list offers «Закрыть» only after every row has settled.
 */
export default function UploadQueue({ items, locale, onDismiss }: Props) {
  ...
  return (
    <div className="mt-2 w-full space-y-2 rounded-md border border-border p-2">
      ...
      {onDismiss && isQueueSettled(items) && (
        <button
          type="button"
          onClick={onDismiss}
